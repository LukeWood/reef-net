...
def main(args):
    ...
    ds, dataset_size = load_reef_dataset(config, min_boxes_per_image=1)  # annotations: [x,y,w,h] in pixels
    ds = ds.shuffle(config.batch_size * 2)
    ds = ds.map(preprocess_data, num_parallel_calls=autotune)  # [x_center, y_center, w, h] in relative
    ds = ds.repeat()
    ds = ds.padded_batch(
